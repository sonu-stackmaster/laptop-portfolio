...
import { Play, Pause, SkipForward, SkipBack, Disc, Volume2, VolumeX, Shuffle, Repeat, Heart, CloudRain, Music } from 'lucide-react';
...
export default function MusicDesktopCard({ isDark, isMuted, onToggleSound }) {
  const [activeTab, setActiveTab] = useState('soundscapes'); // 'soundscapes' or 'player'
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`p-1 rounded-xl border flex items-center justify-between text-xs font-bold ${
        ...
        <button
          onClick={() => { soundFx.playKeyClick(); setActiveTab('soundscapes'); }}
        ...
        <button
          onClick={() => { soundFx.playKeyClick(); setActiveTab('player'); }}
          className={`flex-1 flex items-center justify-center space-x-1.5 py-1.5 rounded-lg transition-all ${
            activeTab === 'player'
              ? isDark ? 'bg-purple-600 text-white shadow-md' : 'bg-orange-500 text-white shadow-md'
              : isDark ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900'
      ...
      {activeTab === 'soundscapes' ? (
        <AmbientSoundscapeCard isDark={isDark} />
      ) : (
        <div className={`p-4 rounded-xl border flex flex-col justify-between text-center select-none ${
          ...
          <div className="my-auto py-3 flex flex-col items-center justify-center relative">
            <div className={`absolute w-28 h-28 rounded-full blur-2xl transition-opacity duration-700 ${
              isPlaying ? 'opacity-80' : 'opacity-20'
            } ${isDark ? 'bg-purple-600' : 'bg-orange-400'}`} />

            <div className={`relative w-28 h-28 rounded-full flex items-center justify-center border-4 shadow-2xl transition-transform duration-700 ${
              isPlaying ? 'animate-spin scale-105' : 'scale-100'
            } ${
              isDark ? 'border-purple-500/40 bg-[#0a0514]' : 'border-orange-400/60 bg-slate-900'
            }`} style={{ animationDuration: '7s' }}>
              <Disc size={70} className="text-purple-400 opacity-80" />
              <div className="absolute w-8 h-8 rounded-full bg-gradient-to-tr from-purple-600 to-pink-500 border-2 border-white flex items-center justify-center shadow-inner">
          ...
          <div className="space-y-0.5 my-2">
            <h3 className={`text-sm font-extrabold font-heading tracking-tight ${
          ...
          <div className="space-y-1 my-2">
            <div className={`w-full h-1.5 rounded-full overflow-hidden cursor-pointer ${
          ...
          <div className="flex items-center justify-around my-2 px-2">
            <button 
              ...
              className={`p-1.5 rounded-full ${isShuffle ? 'text-purple-400 font-bold' : (isDark ? 'text-slate-400' : 'text-slate-600')}`}
            >
              <Shuffle size={14} />
            </button>
            <button onClick={() => soundFx.playKeyClick()} className={`p-1.5 rounded-full ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
              <SkipBack size={18} />
            </button>
            <button
              onClick={togglePlay}
              className={`w-10 h-10 rounded-full flex items-center justify-center text-white shadow-xl active:scale-90 transition-transform ${
            ...
            <button 
              ...
              className={`p-1.5 rounded-full ${isRepeat ? 'text-purple-400 font-bold' : (isDark ? 'text-slate-400' : 'text-slate-600')}`}
            >
              <Repeat size={14} />
            </button>
          </div>
