...
export default function SafariDesktopCard({ isDark }) {
      ...
      <div className={`p-3.5 rounded-xl border space-y-2 ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Globe size={15} className={isDark ? "text-purple-400" : "text-orange-500"} />
        ...
        <p className={`text-xs leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
          Explore open-source cloud microservices and AI application codebases on Sonu&apos;s GitHub.
        </p>
        <button
          onClick={() => handleOpenUrl(personalInfo.github)}
          className={`w-full py-2 rounded-lg text-xs font-bold text-white shadow-md flex items-center justify-center space-x-2 ${
            isDark ? 'bg-gradient-to-r from-purple-600 to-indigo-600' : 'bg-gradient-to-r from-orange-500 to-amber-500'
          }`}
        >
          <Github size={15} />
          <span>Launch GitHub in Browser</span>
        </button>
      </div>
    </div>
  );
